...
export default function AccessoriesImportFormPage() {
  const { data: session } = useSession();
  const router = useRouter();
  const params = useParams();
  const [company, setCompany] = useState<Company | null>(null);
  const [loading, setLoading] = useState(true);
  const [importItems, setImportItems] = useState<AccessoryImportItem[]>([]);
  ...
  const brands = [
    'Alpinestars', 'Dainese', 'Rev\'It', 'Shoei', 'Arai', 'AGV', 'HJC',
    'Icon', 'Bell', 'Scorpion', 'Fox Racing', 'Oakley', 'Generic'
  ];

  const categories = [
    'Helmets', 'Jackets', 'Gloves', 'Boots', 'Pants', 'Body Armor',
    'Back Protectors', 'Knee Guards', 'Sunglasses', 'Bags & Luggage',
    'Tank Bags', 'Tail Bags', 'Phone Mounts', 'GPS Mounts', 'Covers',
    'Locks', 'Tools', 'Cleaning Supplies', 'Other'
  ];
  ...
  const materialOptions = [
    'Leather', 'Textile', 'Mesh', 'Carbon Fiber', 'Plastic', 'Metal',
    'Rubber', 'Foam', 'Kevlar', 'Cordura', 'Nylon', 'Polyester', 'Other'
  ];
  ...
  const addNewItem = () => {
    const newItem: AccessoryImportItem = {
      id: `temp-${Date.now()}`,
      sku: '',
      name: '',
      category: '',
  ...
  const updateItem = (id: string, field: keyof AccessoryImportItem, value: any) => {
    setImportItems(importItems.map(item => 
      item.id === id ? { ...item, [field]: value } : item
    ));
  };

  const validateItem = (item: AccessoryImportItem): { errors: string[], warnings: string[] } => {
    const errors: string[] = [];
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-content" style={{ marginTop: '2rem' }}>
        <div className="content-header">
          ...
          <div className="header-center">
            <h1>Accessories Import Form</h1>
            <p>Add multiple accessories with all required information</p>
          </div>
        </div>
        ...
        {importItems.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Click "Add Accessory" to start importing your accessories inventory</p>
                      ...
                      <div className="form-row">
                        <div className="form-group">
                          ...
                          <input
                            ...
                            className={item.errors?.includes('SKU is required') ? 'error' : ''}
                          />
                        </div>
                        <div className="form-group">
                          <label>Name *</label>
            ...
            <div className="import-actions">
              <div className="import-summary">
                <div className="summary-item">
                  <span className="label">Total Accessories:</span>
                ...
                >
                  {importing ? (
                    ...
                    <>
                      ...
                      Import {importItems.length} Accessories
                    </>
                  )}
                </button>
              </div>
      ...
      {/* Shared styles with other forms */}
      <style jsx>{`
        .content-header {
          margin-bottom: 2rem;
          position: relative;
